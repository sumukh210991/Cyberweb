...
<%def name="headtags()">
	...
	<script type="text/javascript">
		function show_graphs(filename){
			...
			dataString = "path=/home/cyberweb/cwdata/cwusers/mary/ucoam/serucoam_ref_data/dolphin/job.139617/TDATA&filename=" + filename;
			$.ajax({
			    type: "POST",
				url: "/graphsubmenu/drawgraph",
				data: dataString,
				error: function(msg){
				    alert( "Error visualizing data. " + msg);
				},
				success: function(msg){
					$("#graph > .image > .plot_image").html(msg);
				}
			});
		};
...
<!-- For Loop Browser Box -->
  ...
  <div id="graph">
    ...
    <div class="image">
		<h3>Graph Viewer</h3>
		<form name="vdata" method="post" action="">		
			<a href='#' onClick="show_graphs('timer_serUCOAM.dat')">timer_serUCOAM.dat</a>
		</form>
