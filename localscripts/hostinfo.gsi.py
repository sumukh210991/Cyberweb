...
if (len(sys.argv) >= 3):
    ...
    args  = "'hostname; whoami;  date; env"   #note: var does not work in popen...need to escape chars?
    print( 'GSI_CMD: %s %s %s ' % ( gsicmd, hostname, args) )

    try:
       out1 = Popen([gsicmd,hostname,'whoami; hostname; ls -al'],stdout=PIPE).communicate()[0]
